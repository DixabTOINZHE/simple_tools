...
public class stBlockPlacer extends Item {
    ...
    public ActionResult useOnEntity(ItemStack using_stack, PlayerEntity using_player, LivingEntity entity, Hand hand) {
        ...
        if (!world.isClient()) {
            int diameter = using_player.getStackInHand(hand).get(stItemComponents.BLOCK_PLACER_PLACING_DIAMETER);
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!environment.isClient) {
            ...
            //放置
                ...
                if (using_player.isCreative()||(block_is_enough(offhand_itemItem, diameter, using_player_inventory, using_stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE)))){

                    int number_of_placed_block = 0;
                    //水平放置
                    if (using_stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 1){
                    ...
                    //竖起x方向
                    else if (using_stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 2) {
                    ...
                    //竖起z方向
                    else if (using_stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 3) {
    ...
    private boolean change_mode_or_diameter(ItemStack using_stack, PlayerEntity using_player, Hand hand, Text changed_diameter_pre, int diameter, Text changed_diameter_suf, Text place_mode_1, Text place_mode_2, Text place_mode_3, Text place_mode_4){
        ...
        int min = settings.block_placer_min_diameter;
        int max = settings.block_placer_max_diameter;
        ...
        if (Screen.hasAltDown()){
            ...
            using_player.sendMessage(Text.of("--------------------"));
            using_player.getStackInHand(hand).set(stItemComponents.BLOCK_PLACER_PLACING_DIAMETER, stValues.change_block_placer_place_diameter(diameter, min, max));
            using_player.sendMessage(Text.of(changed_diameter_pre.getString()+ using_player.getStackInHand(hand).get(stItemComponents.BLOCK_PLACER_PLACING_DIAMETER)+changed_diameter_suf.getString()));
        ...
        //Ctrl+右键：更改放置模式
            ...
            using_player.sendMessage(Text.of("--------------------"));
            using_player.getStackInHand(hand).set(stItemComponents.BLOCK_PLACER_PLACING_MODE, stValues.change_block_placer_place_mode(using_stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE)));
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 1){
            place_mode_tip_key = "item.simple_tools.block_placer.tip.mode_1";
        } else if (stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 2){
            place_mode_tip_key = "item.simple_tools.block_placer.tip.mode_2";
        } else if (stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 3){
            place_mode_tip_key = "item.simple_tools.block_placer.tip.mode_3";
        } else if (stack.get(stItemComponents.BLOCK_PLACER_PLACING_MODE) == 4){
        ...
        Text tip_diameter = Text.translatable(
                ...
                "\u00A72"+Integer.toString(stack.get(stItemComponents.BLOCK_PLACER_PLACING_DIAMETER))
        );
