...
public class stBlockSearcher extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Text search_diameter=Text.translatable(
                ...
                "\u00A7r"+Integer.toString(using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER)),
                "\u00A7r"+Integer.toString(using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER)),
                "\u00A7r"+Integer.toString(using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER))
        );
        ...
        if(!environment.isClient){
            ...
            using_player.sendMessage(start);{
                ...
                int radius = (using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER)-1)/2;
                    ...
                    for (int dx = -1*radius; dx <= radius; dx++) {
                        for (int dz = -1*radius; dz <= radius; dz++) {
                            ...
                            if (searched_blockItem == offhand_itemItem) {
                                number_of_searched_block = number_of_searched_block + 1;
                            }
                        }
                    }
                }
    ...
    private boolean change_diameter(PlayerEntity using_player, ItemStack using_stack){
        if (Screen.hasAltDown()){
            using_player.sendMessage(Text.of("--------------------"));
            using_stack.set(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER, stValues.change_block_searcher_searching_diameter(using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER)));
            Text changed_diameter = Text.translatable(
                    "item.simple_tools.block_searcher.changed_diameter",
                    Integer.toString(using_stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER))
            );
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        Text tip_diameter = Text.translatable(
                ...
                "\u00A72"+Integer.toString(stack.get(stComponents.BLOCK_SEARCHER_SEARCHING_DIAMETER))+"\u00A72"
        );
