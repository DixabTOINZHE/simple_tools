...
public class stSlimeBlockPlacer extends Item {
    ...
    private void place_slime_block(PlayerEntity using_player, ItemStack used_stack){
        ...
        if (!(using_player.getWorld().isClient())) {
            ...
            if (Screen.hasControlDown()){
                ...
                int min = settings.slime_block_placer_min_diameter;
                int max = settings.slime_block_placer_max_diameter;

                used_stack.set(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER, stValues.change_slime_block_place_diameter(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER), min, max));
                Text changed_place_diameter = Text.translatable(
                        "item.simple_tools.slime_block_placer.changed_place_diameter",
                        Integer.toString(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER))
                );
            ...
            //放置粘液块
            if (number_of_slime_block >= used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER)* used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER)) {

                boolean placed = false;

                //没空间的解决办法：
                if (!can_place(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER), using_player.getWorld(), relative_pos)){
                ...
                for (int dy = 0; dy <= relative_pos.getY() + 64; dy++) {
                    ...
                    if (!can_place(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER), using_player.getWorld(), object_block)) {
                        BlockPos place_pos = object_block.add(0, 1, 0);
                        place(using_player.getWorld(), place_pos, used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER));
                        placed = true;
                        break;
                    }
                }
                ...
                if (!(placed)) {
                    if (using_player.getWorld().getRegistryKey() == World.OVERWORLD){
                        place(using_player.getWorld(), new BlockPos(relative_pos.getX(), -64, relative_pos.getZ()), used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER));
                    } else if ((using_player.getWorld().getRegistryKey() == World.NETHER)||(using_player.getWorld().getRegistryKey() == World.END)){
                        place(using_player.getWorld(), new BlockPos(relative_pos.getX(), 0, relative_pos.getZ()), used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER));
                    }
                }
                ...
                if (!(using_player.isCreative())) {
                    if (using_player.getMainHandStack().getItem() == stTools.SLIME_BLOCK_PLACER){
                        using_player.getMainHandStack().damage(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER)* used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER),using_player, EquipmentSlot.MAINHAND);
                    }
                    using_player.getInventory().remove(stack -> stack.getItem() == Items.SLIME_BLOCK, used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER)* used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER), null);
                }
    ...
    public void appendTooltip(ItemStack used_stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        Text diameter = Text.translatable(
                ...
                "\u00A72"+Integer.toString(used_stack.get(stItemComponents.SLIME_BLOCK_PLACER_PLACING_DIAMETER))
        );
