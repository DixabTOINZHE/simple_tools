...
public class stPlayerStealer extends Item {
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity using_player, LivingEntity object_player, Hand hand) {
        ...
        Text no_item = Text.translatable(
                "item.simple_tools.player_stealer.no_item"
        );
        Text unenabled_error = Text.translatable(
                "item.simple_tools.player_stealer.unenabled_error"
        );
        ...
        if (!(enviroment.isClient())){
            ...
            if (object_player instanceof PlayerEntity) {
                ...
                if (!stWorldJson.read(stWorldJson.get_path(using_player.getServer().getOverworld())).player_stealer_enabled) {
                    ...
                    for (int finder = 0; finder <= 35; finder++) {
                        ...
                        if (!object_stack.isEmpty()) damage++;
                    ...
                    if (damage == 0) using_player.sendMessage(no_item);
                    ...
                    using_player.sendMessage(Text.of("--------------------"));
                    using_player.sendMessage(unenabled_error);
                    using_player.sendMessage(Text.of("--------------------"));
